...
def run_mc_dropout(model, tensor, num_samples=20):
    ...
    confidence = float(mean_prob) if mean_prob >= 0.5 else float(1.0 - mean_prob)
...
def astronomical_validation_heuristics(flux: np.ndarray, period: float, t0: float, duration: float):
    ...
    # 2. V-shape vs U-shape heuristic
    # A grazing eclipsing binary has a V-shaped transit (sharp ingress/egress).
    # A planetary transit has a flat bottom (U-shaped).
    # Slicing the transit center (middle 30% of transit window)
...
def predict_npz(model_path, npz_path, output_dir="experiments/predictions"):
    ...
    mc_prob, confidence, variance, uncertainty, reliability = run_mc_dropout(model, tensor)
    ...
    verdict = validation["verdict"] if validation["override"] is not None else ("Exoplanet Candidate" if final_prob >= 0.5 else "Rejected")
    
    # Save plots
    # Phase fold representation for plotting
...
if __name__ == "__main__":
    ...
    parser.add_argument("model_path", type=str, help="Path to compiled Keras model (.h5)")
